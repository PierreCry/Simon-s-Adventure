...
def transition(event):
    ...
    for k in range (1,4): #Transition entre les différentes map
        if cptmap==0 and (x == X1[k-1] and y == Y1[k-1]): #Transition de la map0 aux différentes maps
            carte(86,324,k)
        if cptmap==k and (x == 54 and y == 324): #Transition des différentes map à la map0
...
def DialogueMag(): #Dialogue avec le magicien
    ...
    if M[1][i][j]==51 and cptM < len(DiagMag3) and élixir()==True: #Le joueur parle pour la deuxième fois au Magicien sur la Map1 après avoir ramassé tous les objets demandé
        text(DiagMag3[cptM])
        if cptM == len(DiagMag3)-1: #Lorsque la liste des dialogues est parcouru, le joueur s'apprête à faire les choix moraux
            ChoixMor1()   
    else:
        if M[1][i][j]==51 and cptM < len(DiagMag2): #Le joueur parle pour la première fois au Magicien sur la Map1
            text(DiagMag2[cptM])
...
def DialogueGard(): #Dialogue avec le garde
    ...
    if M[0][i][j]==49 and cptG < len(DiagGard2) and trinité()==True: #Le joueur parle pour la deuxième fois au Garde après avoir ramassé tous les objets demandé
        text(DiagGard2[cptG])
        if cptG == len(DiagGard2)-1: #Lorsque la liste des dialogues est parcouru, le garde s'en va en laissant un objet à Simon
            inventaire(60)
            changementdec(1)
        ...
        if M[0][i][j]==49 and trinité()==False: #Le joueur parle pour la première fois au Garde
            ...
            if fait!=0: #Le joueur a fait le choix
                if fait==1: #Si le joueur a fait le premier choix
                    ...
                    text(Ch2Gard[cptG])
                    if cptG == len(Ch2Gard)-1:#Lorsque la liste des dialogues est parcouru, le garde meurt en laissant un objet à Simon
                        inventaire(58)
                        changementdec(1)
...
def DialogueVill(): #Dialogue avec le Villageois
    ...
    if M[0][i][j]==50 and cptV < len(DiagVill2) and clénote()==True: #Le joueur parle pour la deuxième fois au Villageois après avoir ramassé tous les objets demandé
        text(DiagVill2[cptV])
        if cptV == len(DiagVill2)-1 and cptenig < len(EnigVillQ): #Lorsque la liste des dialogues est parcouru, le joueur commence les énigmes
            Enigme()
...
def indices(x,y): #Fonction qui parmet d'appeler les indices i et j
    i=(x-50)//32
    j=(y-100)//32
    return i,j

def changementdec(f): #Fonction permettant de remplacer une image par une autre dans la matrice
